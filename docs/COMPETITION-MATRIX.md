...
| Every entry must run on **Nebius Token Factory** or **Nebius AI Cloud** | `src/provider.js` documents a provider seam only | **Not met.** No Nebius account, call, job, endpoint, DevPod, Sandbox, or cloud runtime exists | Account holder must review access, terms, pricing, privacy, and configure a real runtime personally |
...
| Working demo / hosted application or test build | Offline browser app: `index.html`, `src/app.js`, `src/provider.js` | Public deterministic mock; no Nebius/NVIDIA runtime | Validate an actual integrated runtime before making any cloud-runtime claim |
...
| English functionality/implementation explanation | `README.md`, `docs/PROVIDER-CONTRACT.md`, `docs/DISCLOSURE.md` | Public English materials for the deterministic local mock | Update claims after real integration evidence exists |
